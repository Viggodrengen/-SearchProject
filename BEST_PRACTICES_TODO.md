...
- Search ranking is isolated in `SearchApi/Domain/SearchLogic`, which makes it testable without HTTP, Redis or PostgreSQL.
- Search-index persistence is abstracted behind `ISearchIndexRepository`, making the Repository Pattern explicit.
...
- Consider creating separate infrastructure manifests folders for Kubernetes/Compose if the demo expands.
